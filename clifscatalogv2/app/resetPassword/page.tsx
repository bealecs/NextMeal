...
export default function ResetPassword() {
  ...
  return (
    <>
        <Link href={"/"} className={ResetPasswordStyles.back}>← Back to home</Link>
        <section className={ResetPasswordStyles.container}>
            ...
            <form className={ResetPasswordStyles.passwordResetForm}>
                ...
                <input id="passwordReset" value={formInput} onChange={(e) => setFormInput(e.target.value)} type="email" required/>
                <button onClick={handleClick}>{!clickedState ? "Submit Email" : "Submit OTP"}</button>
            </form> 
            :
            <form className={ResetPasswordStyles.passwordResetForm}>
                <label htmlFor="passwordReset">Enter your OTP:</label>
                <input id="passwordReset" value={formInput} onChange={(e) => setFormInput(e.target.value)} type="number" required/>
                <button onClick={handleClick}>{!clickedState ? "Submit Email" : "Submit OTP"}</button>
            </form> }
            <p className={ResetPasswordStyles.infoP}>
                An email will be sent with a one-time-code for you to receive. Once you have your code, enter it in the box and click Submit OTP.
            </p>
            <span className={ResetPasswordStyles.asteriks}>** </span>
            <p className={ResetPasswordStyles.infoWarning}>
                Please do not leave this page once you have gotten your code. 
                If you leave, you will need to re enter your email for a fresh code.
            </p>
        </section>
    </>
  );
