...
export default function Chat(props: Props) {
  ...
  async function getUserPreferences(session: Session) {
    ...
    const selectedPreferences = Object.keys(userPreferences[0]).filter(
      (key) => {
  ...
  const { messages, input, handleInputChange, handleSubmit } = useChat(
    props.session && preferencesToSend !== null
      ? {
          body: {
  ...
  return (
    <section
      className={
        props.session === null
          ? "container_light_chat"
          : theme.themeValue + "_chat"
      }
      id={ChatStyles.container}
    >
      <div className={ChatStyles.messagesContainer}>
        {messages.length < 1 && (
          <div className={ChatStyles.noMessages}>
            ...
            <p>
              Example: Give me a list of 5 Mexican dishes that do not contain
              dairy
            </p>
